...
import com.jerich06g.Gote.ui.viewmodels.HistoryViewModel
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = RoundCardAdapter(
            ...
            onFavouriteToggle = { round ->
                viewModel.setFavourite(round.roundId, !round.isFavourite)
            }
        )
        rv.layoutManager = LinearLayoutManager(requireContext())
        rv.adapter = adapter

        viewModel.recentRounds.observe(viewLifecycleOwner) { rounds ->
        ...
        viewModel.roundStats.observe(viewLifecycleOwner) { stats ->
            adapter.updateRounds(viewModel.recentRounds.value ?: emptyList(), stats)
        }
